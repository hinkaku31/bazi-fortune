...
【鑑定の掟：絶対厳守】
1. データの完全遵守：提供された命式（日主、月支、五行）を絶対の正解とし、勝手な変更や推測で異なる星の話をすることは厳禁です。「身弱」なら「身弱」、「丙」なら「丙」として鑑定してください。
...
「個人の内面、精神性、無意識の反応」に特化。仕事や対人関係の話は他でするため、ここでは「自分自身との向き合い方」「魂の満たし方」に集中すること。`;
                dataFocus = "日主（生まれた日の干）と月令の関係性を最重要視せよ。";
            } else if (topic === 'social') {
                specificInstruction = `【社会的使命（月支）の鑑定】
「仕事場での評価、リーダーシップ、集団内での振る舞い」に特化。内面の話は不要。「どうすれば社会で勝ち上がれるか」「周囲からどう見られているか」に集中すること。`;
                dataFocus = "月支（生まれた月の支）を通変星に変換し、社会運を分析せよ。";
            } else if (topic === 'partner') {
                specificInstruction = `【パートナー（日支）の鑑定】
「恋愛、結婚、家庭」に特化。仕事の話は厳禁。「どのような異性を求めるか」「結婚後の家庭像」「陥りやすい恋愛トラブル」に集中すること。`;
                dataFocus = "日支（配偶者の座）と日主の干合関係を分析せよ。";
            } else if (topic === 'job_success') {
                specificInstruction = `【適職と成功（時支）の鑑定】
「具体的な職業適性、金運、成功の条件」に特化。精神論は不要。「どの業界に行くべきか」「どうやって資産を築くか」に集中すること。`;
                dataFocus = "時柱（晩年・成果）と全体の五行バランスから適職を導き出せ。";
            }

            prompt = `伝説の鑑定士よ、この者の命式（${JSON.stringify(bazi)}）と五行（${JSON.stringify(elements)}）を正しく読み取り、以下の四部構成で「現実的真実」だけを5000文字の気迫で書き尽くしなさい。

【重要：データ整合性】
${dataFocus}
入力されたデータ以外の星（存在しない十二運や通変星）を語ることは許されない。

${specificInstruction}

1. あなたの宿命：命式データに基づき、現実社会で戦うための武器（長所）と弱点（短所）を断定する。
...
占い方の解説は一切不要。抽象的な精神論ではなく、血の通った現実的アドバイスで魂を揺さぶりなさい。執筆開始：`;
            ...
            let timeFocus = "";
            if (topic === 'today') timeFocus = "「今日この24時間」に起きる具体的な出来事と、時間帯別のアドバイス";
            else if (topic === 'tomorrow') timeFocus = "「明日」訪れる具体的なチャンスと、準備すべきこと";
...
命式データ（${JSON.stringify(bazi)}）と五行（${JSON.stringify(elements)}）を背景に、${timeFocus}について書き尽くしなさい。
...
【指示】
この続きから、さらに5000文字の熱量で鑑定を完遂せよ（文字欠け厳禁・データ整合性厳守）：`
                            : prompt
                    }
                ],
                temperature: 0.7,
