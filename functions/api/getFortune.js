...
「具体的な職業適性、金運」に特化。精神論は不要。「どの業界に行くべきか」「どうやって資産を築くか」に集中すること。`;
            ...
            prompt = `伝説の鑑定士よ、この者の命式（日主：${bazi.nicchuu[0]}）を正しく読み取り、以下の四部構成で「真実」を鋭く凝縮して書きなさい。

【重要：文章量の制限と凝縮】
ここでは長文は不要である。各項目を「3〜4行程度の鋭い洞察」でまとめ、合計800文字程度で、しかし急所を突く言葉で記述せよ。
エラー回避のため、無駄な修飾語を削ぎ落とし、結論を急げ。
...
1. あなたの宿命：命式データに基づき、現実社会で戦うための武器（長所）と弱点（短所）を断定する（短く鋭く）。
2. 具体的性格：干合や蔵干の並びがもたらす、具体的な行動パターンや思考の癖を暴き出す（短く鋭く）。
3. 人生の課題：空亡や十二運の象意を、人生の具体的な危機として予言し、回避策を示す（短く鋭く）。
4. 開運の鍵：喜神・忌神に基づき、明日から「何を捨て、何を拾うべきか」を具体的に指示する（短く鋭く）。

占い方の解説は一切不要。要点だけを突き刺すように語りなさい。執筆開始：`;
        } else {
